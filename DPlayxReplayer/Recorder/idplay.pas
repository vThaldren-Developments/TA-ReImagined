...
const
  VERSION = '0.99�2';
  INTERNALVER = 3;  // 0 = ingen/j�ttegammal 1 = 97 2 = 98/99.b1 3 = 99b2
  UNITSPACE = 5000;
...
//    holdstring   : string;
    ...
    adjustcount  : integer;
    notime       : boolean;
    nonserverstart:boolean; //�r detta ett tidigare inspelat spel ?
    use3d        : boolean;
    auto3d       : boolean;
    usecomp      : boolean;
    autorec      : boolean;
    ...
    compatibleTA : boolean; //ta som vi k�nner minnesstrukturen f�r ?
...
begin
  ...
  if not TestByte($47D4C0,$0f) then
    chatview^.myCheats:=chatview^.myCheats or $40;  //bygg �verallt
...
//      filename:='';
  ...
  //          setword (@tmp [2], a + 1 + startid [convertid (from)]);
    ...
    if (copy(s2,1,5)='.take') and not isself(from) then begin
      ...
      //nya recorders vet att man inte ska ge bort units som inte �r klara
...
begin
  ...
  //----------Till�gg f�r att spara varifr�n reccen gjordes----------
  ...
  //Till�gg f�r att spara crc  - #$9 g�r att unitsyncen kommer ignorera den
...
{            tmps:='1234'+#$06;
      ...
      if s[1]=#$8 then begin //b�rja ladda
...
// kefft s�tt att s�tta startid.. damnusj
                {$IFNDEF RELEASE}
                ...
                if not notime then begin
                  ...
                  begin
                    if tmp[3]>char(fastspeed+10) then begin
                      ...
                      tmp := #$2a+'d';          //Ta bort paketet, s� att det inte h�nder n�t
                      datachanged := true;
                    end else
...
//      c := SmartPak (Copy (Result, 8, Length (Result)), from);
      ...
      begin
        ...
        if b<0 then
          b:=b+1000*60*60*24;                   //kl24 fix
        s:=s+char(b and $ff)+char(b shr 8);     //tid sedan senaste
        s:=s+char(convertid(from));             //spelare som s�nde
        s:=s+c;                                 //paketet
        s[1]:=char(length(s) and $ff);          //fyll i storlek
        s[2]:=char(length(s) shr 8);
        logsave.add(s);                            //write a packet
  ...
  if datachanged then
  begin
    //Ett tomt paket g�rs om till ett icketomt
...
begin
  ...
  //till�gg f�r skicka namn till mk
...
{    if stringbuf.Count <> 0 then
    ...
    begin
      ...
      if takefrom<>0 then begin
        ...
        wp^:=w;
        ip:=@holdstring[3];
        ip^:=playerid[1];                                                          //�ndrat fr�n 7E
        holdstring:=#$14+holdstring+#$00#$00#$00#$00+'F'+#$01#$00#$00#$00#$00+'d'+#$7F#$00#$00#$00#$00#$00;
        setword(@holdstring[12], health[w - 1]);

        //Bara take p� unitar som �r f�rdigbyggda
        if donestatus [w - 1] = 0 then
          SendLocal (holdstring, 0, true, false); //kopierar den hit och �ndrar nedan till maxunits-2 fr�n maxunits-1
