...
const
  NUM_FAKES = 20;
  KEEP_LIMIT = 200;     //Borde vara typ 20 sek? jaja på denna tid måste nissen ha units iaf.
  KILL_SPEED = 20;
  GIVE_SPEED = 20;
...
type
  TPlayer = class
    Id  :TDPID;
    Data :TIdent;
    Synced :integer;          //I vilket stadium av synkning man är typ.. 0 = inte
    Sent :integer;            //Antalet $1a som skickats till denna svenne
    curunit :integer;         //Vilken unit vi ska skicka
    errors :integer;          //Antal felaktiga units
    acked  :integer;          //Antal units som klienten svarat med crc på
    received :integer;        //Antalet $1a som den påstår sig tagit emot
    hastaken :boolean;        //True om denna nisse tagit kontrollen över någon
    warned :boolean;          //true om denna svenne blivit dissad
  end;
...
//    player :TDPID;
    ...
    unitrange :array[1..10] of integer;     //Startbasen för en viss spelare.. tex 0, 250, etc. baserat på maxunits..
    ...
    lastplayerpack :cardinal; //senaste 2c serienummer för watcher
    lastcompack :cardinal; //lägsta 2c serienummer för alla drones
    lastdronepack : array[1..10] of cardinal; //senaste 2c för drone
    possynccomplete : array[1..10] of cardinal; //när börja sända efter en pos
    recentpos       : array[1..10] of boolean;
    kill :TList;    //Lista med units som ska dö en ond bråd död
    take :TList;    //Lista med units som ska få en ny master
...
{kol 2 har inte med metall / energy att göra iaf}

{ bit 1 = den till höger}
{kol 3 : loword * 100 = energy, hiword * 100 = metal}
{kol 1 bit 32 = game closed}
{loword kol 4 = antal units}
          ...
          $1a : begin
                  ...
                  case upkt.subtype of
                    $2 :begin
                          ...
                          if Assigned (runit) then  //If not then ointressant, det är en unit vi inte känner till
                          begin
...
{    if err <> DP_OK then
  ...
  begin
    ...
    begin
      ...
      id.InternVer := 0;  //onödigt att recodern varnar om att den inspelade tänker fuska :)
      id.AllowWatch := true;
//      id.PermLos := true;
...
{        if isdrone (playr.id) then
      ...
      case playr.synced of
        0  :begin
              ...
              p.Free;
              playr.synced := 1;
//              playr.sent := 1;    //Totala antalet $1a som skickats
            end;
        1  :begin
{              tmp := '';
...
{    if done then
  ...
  begin
    ...
    quit := false;    //används i DoPlay sen


    //Så vi vet vilken unitnrbas de har
    players.sort;
