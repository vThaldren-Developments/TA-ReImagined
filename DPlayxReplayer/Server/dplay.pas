 ...
 *
 *  DirectX 5 Delphi adaptation by Erik Unger and David Sisson
 *
 *  Modyfied: 20.7.98
 *
...
{$ENDIF}
    ...
    function GetPlayerName(idPlayer: TDPID; lpData: Pointer; var lpdwDataSize: DWORD)
        : HResult;
        {$IFDEF D2COM} virtual; stdcall; abstract; {$ELSE} stdcall; {$ENDIF}
    function GetSessionDesc(lpData: Pointer; var lpdwDataSize: DWORD) : HResult;
        {$IFDEF D2COM} virtual; stdcall; abstract; {$ELSE} stdcall; {$ENDIF}
...
initialization
begin
//  Lib := LoadLibrary ('DPlayX.dll');

  GetMem (p, 1000);

  GetSystemDirectory (p, 1000);
  s := p;
//  FreeMem (p, 1000);                        //Kommenteras denna in kraschar allt!?!?!?!?!?!
  s := s + '\dplayx.dll';
