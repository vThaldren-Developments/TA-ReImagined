...
Section "TA Demo Recorder (required)"
  ...
  ; Skriv ner en regkey som denna installer kikar p� sen
  ...
  ; Skriv uninstaller
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\TA Demo Recorder" "TA Demo Recorder 0.99�2" "TA Demo Recorder (remove only)"
...
Section "Uninstall"
  ...
  ; Saker ur underkataloger kan vi k�ra *.* p�
  Delete $INSTDIR\Docs\*.*
  Delete $INSTDIR\bitmaps\*.*

  ; Men i huvudkatalogen k�nns b�st att specificera
  ...
  ; Sj�lva installern ska ocks� bort
...
; Saker f�r att hantera v�ra custom dialogrutor
...
; L�gger resultatet fr�n CallInstDLL p� stacken innan avslut
Function RunConfigure
  ...
  ; L�gg in katalognamnet i global $6 och "fixa-pathar" i $5
  ReadINIStr $6 $7 "Field 3" State
  ReadINIStr $5 $7 "Field 5" State

  ; Om man tryckt p� cancel s� ska det avslutas
  StrCmp $R0 "cancel" "" nocancel
    Call Cleanup
    Quit
  nocancel:

  ; Trycker man p� back s� beh�ver man inte validera katalogen
  StrCmp $R0 "back" "" noback
    Push "back"
    Return 
  noback:

  ; Men trycker man p� next m�ste man ha valt en giltig TA-katalog, annars k�rs v�ljsidan igen
